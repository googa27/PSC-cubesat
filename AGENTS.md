...
`PSC-cubesat` is classified as `Hardware / Docs Legacy` under Portfolio Project #24. Preserve public/upstream compatibility, privacy, and evidence boundaries; do not infer maturity beyond executable tests.
...
This repository follows [Portfolio Project #24](https://github.com/users/googa27/projects/24) and [PSC-cubesat rollout issue](https://github.com/googa27/PSC-cubesat/issues/1). A repository-specific, evidence-backed exception in `docs/ARCHITECTURE.yaml` may specialize a rule; undocumented drift is not an exception.
...
Repository posture: No finance core coupling. Data posture: Telemetry/document sources require schema/provenance plan only upon revival.
