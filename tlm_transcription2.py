...
#translation of measures from bits to degrees/second
def gyrometre_traduire(string):
    pass
...
def traiter(data):
    ...
    aux = []
    for i in range(3, 6):
        aux.append((int(data[i], 16) - 2**7)*0.14)#degrees
    ...
    aux = []
    for i in range(9, 15):
        aux.append(int(data[i], 16)*12.89*1E-3)#micro tesla
    out.append(np.array(aux))
    return out

def ligne_to_data(ligne):
    pass
